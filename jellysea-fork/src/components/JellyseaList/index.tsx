...
interface JellyseaListProps {
  items: { id: number; mediaType: string; posterPath?: string; voteAverage?: number; mediaInfo?: { status?: number }; title?: string; name?: string; releaseDate?: string; firstAirDate?: string; overview?: string }[]
  isLoading: boolean
...
export default function JellyseaList({ items, isLoading, isLoadingMore, isReachingEnd, isEmpty, fetchMore }: JellyseaListProps) {
        ...
        return (
          <li key={item.id}>
            <TitleCard
              ...
              year={isMovie ? item.releaseDate?.split('-')[0] : item.firstAirDate?.split('-')[0]}
