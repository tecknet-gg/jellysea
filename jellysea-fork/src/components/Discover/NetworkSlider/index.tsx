...
import CompanyCard from '@app/components/Discover/CompanyCard'
...
export default function NetworkSlider() {
  const items = networks.map((network, index) => (
    <CompanyCard key={`network-${index}`} name={network.name} image={network.image} url={network.url} />
  ))
