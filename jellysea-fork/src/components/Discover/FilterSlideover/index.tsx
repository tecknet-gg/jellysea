...
const FilterSlideover = ({
  ...
  const mediaStatus = (router.query.mediaStatus as string) ?? '';
  ...
  return (
    <SlideOver
      ...
      subText={intl.formatMessage(messages.activefilters, {
        count: countActiveFilters(currentFilters) + (mediaStatus ? 1 : 0),
      })}
      onClose={() => onClose()}
    >
      <div className="flex flex-col space-y-4">
        ...
        <select
          value={mediaStatus}
          onChange={(e) =>
            updateQueryParams('mediaStatus', e.target.value || undefined)
          }
          className="w-full rounded-lg border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-white focus:border-accent-500 focus:outline-none"
        >
        ...
        <div className="pt-4">
          <Button
            className="w-full"
            disabled={
              Object.keys(currentFilters).length === 0 && !mediaStatus
            }
            onClick={() => {
