      ...
      <div className="flex items-center space-x-4 px-6 py-4 text-sm font-medium leading-5">
        <div
          className={`flex h-10 w-10 flex-shrink-0 items-center justify-center border-2 ${
            active ? 'border-accent-600' : 'border-white'
          } ${completed ? 'border-accent-600 bg-accent-600' : ''} rounded-full`}
        >
          {completed && <CheckIcon className="h-6 w-6 text-white" />}
          {!completed && (
            <p className={active ? 'text-white' : 'text-accent-200'}>
              {stepNumber}
            </p>
          )}
        </div>
        <p
          className={`text-sm font-medium leading-5 ${
            active ? 'text-white' : 'text-accent-200'
          }`}
        >
          {description}
        </p>
