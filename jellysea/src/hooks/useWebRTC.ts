...
export function useWebRTC({
  ...
  }, [signalingUrl, userInfo, myPeerId, updatePeerStates, onChatMessage])

  const createPeerConnection = useCallback((targetId: string) => {
    const pc = new RTCPeerConnection(ICE_SERVERS)
    pcMap.current.set(targetId, pc)

    const dc = pc.createDataChannel('jellysea-vp', {
      ordered: true,
    })
  ...
  const joinRoom = useCallback(async (targetRoomId: string): Promise<void> => {
    return new Promise((resolve, reject) => {
      const handler = (event: MessageEvent) => {
        ...
        if (msg.type === 'error') {
          wsRef.current?.removeEventListener('message', handler)
          const errMsg = (msg.payload as { message: string }).message
